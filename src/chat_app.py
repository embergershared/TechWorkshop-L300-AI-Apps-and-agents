...
from utils.response_utils import (
    ...
    extract_product_names_from_response,
)
...
async def get_cached_image_description(image_url: str, image_cache: dict) -> str:
    ...
    if image_url in image_cache:
        logger.info(
            "Using cached image description",
            extra={"url": image_url[:50], "cache_size": len(image_cache)},
        )
    ...
    try:
        ...
        description = await loop.run_in_executor(
            thread_pool, get_image_description, image_url
        )
...
async def pre_fetch_image_description(image_url: str, image_cache: dict):
    ...
    if image_url and image_url not in image_cache:
        ...
        try:
            ...
            image_cache[image_url] = description
            logger.info(
                "Pre-fetched and cached image description",
                extra={"url": image_url[:50]},
            )
        except Exception as e:
            logger.error(
                "Failed to pre-fetch image description",
                extra={"url": image_url[:50], "error": str(e)},
            )
...
async def get():
    chat_html_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "chat.html"
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    customer_loyalty_executed = (
        False  # Flag to track if customer loyalty task has been executed
    )
    ...
    loyalty_response_sent = (
        False  # Flag to track if loyalty response has been sent to user
    )
    ...
    async def run_customer_loyalty_task(customer_id):
        ...
        with tracer.start_as_current_span("Run Customer Loyalty Thread"):
            ...
            bot_reply = ""
            async for msg in processor.run_conversation_with_text_stream(
            ...
            session_loyalty_response = (
                parsed_response  # Store the full response for later
            )
            # Do NOT send the response here!
            log_timing(
                "Customer Loyalty Task",
                start_time,
    ...
    try:
        while True:
            ...
            chat_history = parse_conversation_history(
                conversation_history, chat_history, user_message
            )

            # The following lines of code is responsible for serving all results.
            ...
            #         fast_json_dumps(
            #             {"answer": response, "agent": "single", "cart": persistent_cart}
            #         )
            #     )
            # except Exception as e:
            #     logger.error(
            ...
            try:
                ...
                agent_selected = validated_env_vars.get(
                    agent_name
            ...
            try:
                ...
                if image_url:
                    ...
                    log_cache_status(image_cache, image_url)
                    image_data = await get_cached_image_description(
                ...
                if agent_name in [
                    ...
                    log_timing(
                        ...
                        product_start_time,
                        f"Found: {len(products) if products else 0}",
                    )
                ...
                if image_data or products:
                    ...
                    if products:
                        context_parts.append(
                            f"Available products: {fast_json_dumps(products)}"
                        )
                ...
                with tracer.start_as_current_span(f"{agent_name.title()} Agent Call"):
                    ...
                    if agent_name == "interior_designer_create_image":
                        ...
                        if persistent_image_url:
                            image_data = await get_cached_image_description(
                                persistent_image_url, image_cache
                            )
                            enriched_message = f"{user_message} {image_data}"

                        # Create image using gpt-image-1
                        image = create_image(
                            text=enriched_message, image_url=persistent_image_url
                        )
                ...
                parsed_response = parse_agent_response(bot_reply)
                parsed_response["agent"] = (
                    agent_name  # Override agent field to show which agent responded
                )
                ...
                if agent_name == "cart_manager" and "cart" in parsed_response:
                    if isinstance(parsed_response.get("cart"), list):
                        ...
                        logger.info(
                            f"Cart updated by cart_manager: {len(persistent_cart)} items"
                        )
                ...
                if session_discount_percentage and not parsed_response.get(
                    "discount_percentage"
                ):
                ...
                if (
                    ...
                    and session_loyalty_response
                    and not loyalty_response_sent
                ):
                    ...
                    await websocket.send_text(
                        fast_json_dumps(loyalty_response_with_cart)
                    )
